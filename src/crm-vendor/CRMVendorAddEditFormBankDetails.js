import React from "react";
import {
  Paper,
  Grid,
  Box,
  Typography,
  TextField,
  MenuItem,
} from "@mui/material";
import { nimbleX360Api, nimbleX360WrapperApi } from "common/StoreQuerySlice";

import InputAdornment from "@mui/material/InputAdornment";
import FormatToNumber from "common/FormatToNumber";
import { LoadingButton } from "@mui/lab";

function CRMVendorAddEditFormBankDetails({ formik, isEdit }) {
  const { data: bankList } = nimbleX360Api.useGetBanksQuery();

  const bankCode = bankList?.filter(
    (e) => e.id === formik.values?.bankId
  )?.[0]?.bankSortCode;

  const [
    validateBank,
    {
      data: otherBankDetails,
      isLoading: otherBankDetailsIsLoading,
      error: otherBankDetailsIsError,
    },
  ] = nimbleX360WrapperApi.useNameEnquiryMutation();

  console.log("otherBankDetails", otherBankDetails);

  const verifyBank = () => {
    validateBank({
      bankCode: bankCode,
      accountNumber: formik.values?.accountnumber,
    });
  };

  const bankErrorMessage = otherBankDetailsIsError?.data?.Message;

  React.useEffect(() => {
    if (otherBankDetails?.data) {
      formik.setFieldValue(
      "accountname",
      `${otherBankDetails?.data?.accountName} `
    );    
    }

    formik.setFieldValue("active", true);

    // eslint-disable-next-line
  }, [otherBankDetails]);

  return (
    <div>
      <Paper className="my-10 py-10 px-5 rounded-md">
        <div className="max-w-3xl">
          <div className="max-w-xl">
            <Typography variant="h5">
              <b>Bank Details</b>
            </Typography>
            <Typography>
              Ensure you enter correct information.
            </Typography>
          </div>

          <Box mt={5}>
            <Grid container spacing={2} mt={1}>
              <Grid item xs={12} md={6}>
                <Box>
                  <TextField
                    fullWidth
                    label="Bank Name"
                    select
                    {...formik.getFieldProps("bankId")}
                    value={formik.values?.bankId || ""}
                    error={
                      !!formik.touched?.bankId &&
                      !!formik.errors?.bankId
                    }
                    helperText={
                      !!formik.touched?.bankId &&
                      formik.errors?.bankId
                    }
                  >
                    {bankList &&
                      bankList.map((option) => (
                        <MenuItem key={option.id} value={option.id}>
                          {option.name}
                        </MenuItem>
                      ))}
                  </TextField>
                </Box>
              </Grid>
            </Grid>

            <Grid container spacing={2} mt={1}>
              <Grid item xs={12} md={6}>
                <Box>
                  <TextField
                    fullWidth
                    label="Account Number*"
                    inputProps={{
                      maxLength: 10,
                      inputComponent: FormatToNumber,
                    }}
                    InputProps={{
                      endAdornment: (
                        <>
                          <InputAdornment position="end">
                            <LoadingButton
                              loading={otherBankDetailsIsLoading}
                              disabled={
                                otherBankDetailsIsLoading ||
                                formik.values?.accountnumber
                                  .length !== 10
                              }
                              onClick={() => verifyBank()}
                              size="small"
                            >
                              verify
                            </LoadingButton>
                          </InputAdornment>
                        </>
                      ),
                    }}
                    {...formik.getFieldProps("accountnumber")}
                    error={
                      (!!formik.touched?.accountnumber &&
                        !!formik.errors?.accountnumber) ||
                      bankErrorMessage
                    }
                    helperText={
                      (!!formik.touched?.accountnumber &&
                        formik.errors?.accountnumber) ||
                      bankErrorMessage
                    }
                  />
                </Box>
              </Grid>

              <Grid item xs={12} md={6}>
                <Box>
                  <TextField
                    fullWidth
                    label="Account Name"
                    {...formik.getFieldProps("accountname")}
                    error={
                      !!formik.touched?.accountname &&
                      !!formik.errors?.accountname
                    }
                    helperText={
                      !!formik.touched?.accountname &&
                      formik.errors?.accountname
                    }
                    inputProps={{ readOnly: true }}
                    focused
                  />
                </Box>
              </Grid>
            </Grid>
          </Box>
        </div>
      </Paper>
    </div>
  );
}

export default CRMVendorAddEditFormBankDetails;
