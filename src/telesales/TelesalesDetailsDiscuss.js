...
import { sequestTeleSalesApi } from "./TeleSalesStoreQuerySlice";
...
import { sequestRecoveryApi } from "recovery/RecoveryStoreQuerySlice";

function TelesalesDetailsDiscuss(props) {
  const { recoveryQueryResult } = props;
  const { enqueueSnackbar } = useSnackbar();
  const authUser = useAuthUser();

  const [addRecoveryReplyMutation, addRecoveryReplyMutationResult] =
    sequestTeleSalesApi.useAddExceptionReplyMutation();

  const _recoveryId = recoveryQueryResult?.data?.data?.exceptionDetails?.exceptionId;

  const _isLogger = recoveryQueryResult?.data?.data?.exceptionDetails;

  const discourseData = [
    ...(recoveryQueryResult?.data?.data?.discourseList || []),
  ];

  // const recoverytStatusByActor =
  //   sequestTeleSalesApi.useGetRequestStatusByActorQuery(
      ...
      {() => (
        <div className="overflow-auto relative" style={{ maxHeight: 400 }}>
          ...
          >
            ...
            >
              {exceptionStatusOption.map((option) => (
...
export default TelesalesDetailsDiscuss;
