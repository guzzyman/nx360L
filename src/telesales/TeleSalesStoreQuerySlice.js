...
export const nx360TeleSalesApi = nimbleX360Api.injectEndpoints({
  endpoints: (builder) => ({
    getAllTelesales: builder.query({
      query: ({ ...params }) => ({
        url: "/telesales",
        params: {
          orderBy: "id",
    ...
    getAllSalesById: builder.query({
      query: (id, ...params) => ({ url: "/telesales/" + id + "/15" }),
      providesTags: (data) => [
        {
