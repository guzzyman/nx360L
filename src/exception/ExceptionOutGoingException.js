...
function ExceptionOutGoingException(props) {
  const { outgoingExceptionsResult, isLoading, isError, refetch, ButtonBase } = props;
  const outgoingExceptionsTableInstance = useTable({
    columns,
    data: outgoingExceptionsResult,
    manualPagination: true,
    totalPages: outgoingExceptionsResult?.totalFilteredRecords,
  });
  const navigate = useNavigate();
  return (
    <>
      <DynamicTable
        instance={outgoingExceptionsTableInstance}
        loading={isLoading}
        error={isError}
        onReload={refetch}
...
export default ExceptionOutGoingException;
