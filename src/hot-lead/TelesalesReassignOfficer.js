...
function TelesalesReassignOfficer(props) {
  const { onClose, clientId, ...rest } = props;
  const [addMutation, { isLoading }] =
    nx360HotleadsApi.useReassignCollectionOfficerMutation();
  const templateOptionData = nx360HotleadsApi.useGetUsersQuery();
  const filteredTemplateOptionData = useMemo(
    () =>
      templateOptionData?.data?.filter(
        (obj) => obj?.staff?.collectionOfficer === true
      ),
    [templateOptionData?.data]
  );
  ...
  return (
    <Modal
      onClose={onClose}
      size=""
      title="Reassign Collection Officer"
      {...rest}
    >
      <TextField
...
export default TelesalesReassignOfficer;
