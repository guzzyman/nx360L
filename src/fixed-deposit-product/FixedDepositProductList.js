...
function FixedDepositProductList(props) {
  ...
  const { data, isLoading, isError, refetch } =
    nxFixedDepositProductApi.useGetFixedDepositProductsQuery({
      // offset,
      // limit,
      // ...(debouncedQ
      //   ? {
    ...
    <>
      ...
      >
        <Button
          ...
          onClick={() => navigate(RouteEnum.FIXED_DEPOSIT_PRODUCT_ADD)}
        >
      ...
      <Paper className="p-4">
        ...
        <DynamicTable
          ...
          rowProps={(row) => ({
            onClick: () =>
              navigate(
                generatePath(RouteEnum.FIXED_DEPOSIT_PRODUCT_EDIT, {
                  id: row.original.id,
                })
              ),
          })}
...
export default FixedDepositProductList;
