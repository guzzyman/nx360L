    ...
    <>
      <Paper className="p-4 mb-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4" style={{ maxWidth: 500 }}>
          ...
          >
            {fixedDepositProductTemplate?.interestCompoundingPeriodTypeOptions?.map(
              (option) => (
          ...
          >
            {fixedDepositProductTemplate?.interestPostingPeriodTypeOptions?.map(
              (option) => (
                <MenuItem key={option.id} value={option.id}>
                  {option.value}
                </MenuItem>
          ...
          >
            {fixedDepositProductTemplate?.interestCalculationTypeOptions?.map(
              (option) => (
                <MenuItem key={option.id} value={option.id}>
                  {option.value}
                </MenuItem>
          ...
          >
            {fixedDepositProductTemplate?.interestCalculationDaysInYearTypeOptions?.map(
              (option) => (
                <MenuItem key={option.id} value={option.id}>
                  {option.value}
                </MenuItem>
