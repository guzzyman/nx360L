...
export default function CRMClientFixedDepositUndoApprovalAction (props) {
  ...
  const formik = useFormik ({
    ...
    onSubmit: async values => {
      try {
        ...
        enqueueSnackbar (`Undo Approval Successful!`, {
          variant: 'success',
        });
        onClose ();
      } catch (error) {
        enqueueSnackbar (`Undo Approval Failed!`, {
          variant: 'error',
        });
  ...
  return (
    <Modal onClose={onClose} size="md" title="Undo Approval" {...rest}>
      <TextField
        label="Note"
        multiline
        rows={3}
