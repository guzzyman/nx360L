...
function ClientXLeadXEDRLoanFundWallet({ data, onClose }) {
  ...
  const templateQueryResult =
    nxClientXLeadXEDRLoanApi.useGetClientXLeadXEDRLoanTransactionTemplateQuery({
      savingsAccountId: data?.transaction,
      // command: "deposit",
    });
  ...
  useEffect(() => {
    dataRef.current.formik.setValues({
      ...
      accountNumber: template?.accountNo || "",
...
export default ClientXLeadXEDRLoanFundWallet;
