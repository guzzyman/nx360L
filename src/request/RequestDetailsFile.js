...
function RequestDetailsFile(props) {
  const { requestQueryResult } = props;
  const [view, setView] = useState("list");
  const images = requestQueryResult?.data?.data?.images;
  const imagesWithAttachment = useMemo(() => images?.filter((image) => !!image?.attachment),[images]);
...
export default RequestDetailsFile;
